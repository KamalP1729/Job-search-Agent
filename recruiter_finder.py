...
Falls back to automatic domain inference when company isn't in the local domain map.
"""
...
def find_contacts(company: str, domain: str | None = None) -> dict:
    ...
    # Try explicit / mapped domain first
    if domain:
        result = find_by_domain(domain)
        result["company"] = company
        return result

    # Infer domain candidates and try each until we find contacts
    ...
    for candidate in candidates:
        try:
            ...
            continue  # domain didn't resolve — try next
