...
from job_search import _build_queries, _fetch_jobs, _parse_job, _requires_visa, _yoe_mismatch, _is_stale  # reuse fetch logic
...
def search_and_rank_semantic(
        ...
        try:
            ...
            for raw in raw_jobs:
                ...
                if _requires_visa(job):
                    logger.info("[visa]  %s — %s", job["company"], job["title"])
                    _counts["visa"] += 1
                    continue
                if _yoe_mismatch(job, profile.get("total_yoe", 0)):
                    logger.info("[yoe]   %s — %s", job["company"], job["title"])
                    _counts["yoe"] += 1
                    continue
                if _is_stale(job):
                    logger.info("[stale] %s — %s (%s)", job["company"], job["title"], job.get("posted_at", "?"))
                    _counts["stale"] += 1
                    continue
                seen.add(key)
                jobs.append(job)
    ...
    logger.info(
        "Filter summary — visa: %d | yoe: %d | stale: %d | dup: %d | kept: %d",
        _counts["visa"], _counts["yoe"], _counts["stale"], _counts["dup"], len(jobs),
    )
